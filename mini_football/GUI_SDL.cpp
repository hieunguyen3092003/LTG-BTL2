	...
	if (SDL_PollEvent(&event))
	{
		...
		case SDL_MOUSEBUTTONUP:
		{
			if (event.button.button == SDL_BUTTON_LEFT)
			{
				...
				if (event.button.y > 0 && event.button.y < 141
					&& event.button.x > 0 && event.button.x < 184)
					return pvp;
				if (event.button.y > 708 && event.button.y < 808
					&& event.button.x > 288 && event.button.x < 474)
					return play;
...
void GUI_SDL::draw_ball(ball_type type)
{
	if (type == normal) {
		_ball = IMG_LoadTexture(_rend, "res/ball.png");
	}
	else if (type == ice) {
		_ball = IMG_LoadTexture(_rend, "res/iceball.png");
	}
	else {
		_ball = IMG_LoadTexture(_rend, "res/fireball.png");
	}
	if (!_ball)
	{
		std::cerr << "IMG error: " << IMG_GetError() << std::endl;
...
void GUI_SDL::new_game(bool hard)
{
	...
	SDL_RenderClear(_rend);
	SDL_RenderCopy(_rend, _background, NULL, NULL);
	_background = IMG_LoadTexture(_rend, "res/menu.png");
	if (!_background)
	{
		std::cerr << "IMG error: " << IMG_GetError() << std::endl;
		exit(2);
